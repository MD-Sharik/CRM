...
function EmployeeLogin() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch("http://localhost:3000/api/v1/agent/login", {
        method: "POST",
        headers: {
  ...
  return (
    <div>
      {/* component */}
      <div className="bg-blue-400 h-screen overflow-hidden flex items-center justify-center">
        <div className="bg-white lg:w-5/12 md:6/12 w-10/12 shadow-3xl rounded-md">
          <div className="bg-gray-800 text-white absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-full p-4 md:p-8 md:px-14">
            Agent
          </div>
